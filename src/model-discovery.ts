...
function metadataToCachedModel(
  ...
  const hasNativeCapabilities =
    nativeCapabilities !== null && nativeCapabilities.length > 0;
  ...
  if (hasNativeCapabilities) {
    ...
    else if (nativeCapabilities.includes("image_generation"))
  ...
  const inputCostPerMillion =
    showPricing && metadata?.input_cost_per_token
      ? metadata.input_cost_per_token * 1_000_000
      : undefined;
  const outputCostPerMillion =
    showPricing && metadata?.output_cost_per_token
...
export async function discoverModelsForProvider(
  ...
  try {
    ...
    for (const id of modelIds) {
      try {
        const model = await getModelMetadataAndMode(
          provider,
          id,
          providerName,
          showPricing,
...
async function getEnrichedModelsForProvider(
  ...
  if (config.fetchModels === false) {
    ...
    models = cached
      ? cached.models
      : await discoverModelsForProvider(providerName, config);
  }
...
export async function getAllAvailableEmbeddingModels(): Promise<
  DiscoveredModel[]
> {
  const providers = aiSettings?.providers as ProvidersConfig | undefined;
  if (!providers) return [];
...
export async function refreshModelListCommand(): Promise<void> {
  ...
  if (!providers || Object.keys(providers).length === 0) {
    await editor.flashNotification(
      "No providers configured under ai.providers.",
      "warning",
    );
    return;
  }
...
export function formatModelHint(model: DiscoveredModel): string {
  ...
  if (model.maxInputTokens) {
    const ctx =
      model.maxInputTokens >= 1_000_000
